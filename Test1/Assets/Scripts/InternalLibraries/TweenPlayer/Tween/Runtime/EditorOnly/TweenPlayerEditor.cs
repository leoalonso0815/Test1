...
namespace UnityExtensions.Tween
{
    public partial class TweenPlayer
    {
        ...
        internal class Editor : BaseEditor<TweenPlayer>
        {
            ...
            {
                get { return EditorGUIUtility.isProSkin ? new Color(0.75f, 0.75f, 0.75f, 0.35f) : new Color(0.25f, 0.25f, 0.25f, 0.25f); }
            }

            internal static Color progressBackgroundValid
            {
                get { return EditorGUIUtility.isProSkin ? new Color(0.71f, 0.71f, 0.71f) : new Color(0.25f, 0.25f, 0.25f); }
            }
            ...
            void OnEnable()
            {
                _playAutomatically = serializedObject.FindProperty(nameof(_playAutomatically));
            ...
            {
                float lastTime = 0f;
                for (var i = 0; i < target._animations.Count; i++)
                {
                    var ani = target._animations[i];
                    lastTime = lastTime < ani.maxNormalizedTime ? ani.maxNormalizedTime : lastTime;
                }

                var durationOld = target.duration;
                target.duration = durationOld * lastTime;
                for (var i = 0; i < target._animations.Count; i++)
                {
                    var ani = target._animations[i];
                    var max = ani.maxNormalizedTime;
                    var min = ani.minNormalizedTime;
                    ani.maxNormalizedTime = max * durationOld / target.duration;
                    ani.minNormalizedTime = min * durationOld / target.duration;
                }
            }

            public void Add()
            {
                if (target.timeAdded <= 0f)
                {
                    return;
                }
                var durationOld = target.duration;
                var durationNew = durationOld + target.timeAdded;
                target.duration = durationNew;
                for (var i = 0; i < target._animations.Count; i++)
                {
                    var ani = target._animations[i];
                    var min = ani.minNormalizedTime;
                    var max = ani.maxNormalizedTime;
                    ani.minNormalizedTime = min * durationOld / durationNew;
                    ani.maxNormalizedTime = max * durationOld / durationNew;
                }
            }
            ...
            public override void OnInspectorGUI()
            {
                ...
                {
                    EditorGUILayout.PropertyField(_playAutomatically);
                    ...
                    if (GUILayout.Button("裁切剩余空白时间"))
                    {
                        Crop();
                    }

                    EditorGUILayout.BeginHorizontal();
                    if (GUILayout.Button("追加时间:", GUILayout.Width(200f)))
                    {
                        Add();
                    }
                    target.timeAdded = EditorGUILayout.FloatField(target.timeAdded);
                ...
                if (target._animations != null)
                {
                    ...
                    {
                        var animation = target._animations[i];
                        animation?.OnInspectorGUI(i, target, _animationsProp.GetArrayElementAtIndex(i));
                    }
                }

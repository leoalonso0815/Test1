...
namespace UnityExtensions.Tween
{
    ...
    public class TweenBehaviourEnabled : TweenFloat<Behaviour>
    {
        ...
        public override float current
        {
            ...
            set
            {
                if (target)
                {
                    // target.enabled = target.enabled ? (value >= criticalValue) : (value > criticalValue);
                }
            }
        }
    ...
    public class TweenGameObjectActive : TweenFloat<GameObject>
    {
        ...
        public override float current
        {
            ...
            set
            {
                if (target)
                {
                    var isActive = target.activeSelf ? (value >= criticalValue) : (value > criticalValue);
                    if (target.activeSelf != isActive)
                    {
                        if (null == tweenPlayer)
                        {
                            tweenPlayer = target.GetComponent<TweenPlayer>();
                        }

                        if (null != tweenPlayer)
                        {
                            tweenPlayer.enabled = isActive;
                        }
                    }
        ...
        {
            EditorGUILayout.PropertyField(property.FindPropertyRelative(nameof(criticalValue)));
            EditorGUILayout.PropertyField(property.FindPropertyRelative(nameof(autoPlay)));
            base.OnPropertiesGUI(player, property);
        }
#endif
    ...
    [Serializable, TweenAnimation("Miscellaneous/TimeScale", "Game Speed")]
    public class TweenTimeScale : TweenFloat<Transform>
    {
        ...
        public override float current
        {
            ...
            {
                timeScale = value;
                Time.timeScale = TweenPlayer.GlobalSpeed * timeScale;
            }
        }
    }
